...
// runtime waveforms (loaded from SRAM on init, can be modified)
uint8_t wavData[256] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
...
/**
 * Checks for a sysex message to overwrite the current wav shape with a custom
 * 16 byte payload
 */
void updateWavSysex(void) {
  if (sysexPayload.ready) {
    sysexPayload.ready = false;
    // EMU_printf("SYSEX read %d bytes for channel %d\n", sysexPayload.size,
    //            sysexPayload.channel);

    if (sysexPayload.id == SYSEX_NON_COMMERCIAL &&
        sysexPayload.device_id == SYSEX_MGB_ID && sysexPayload.channel == WAV &&
        sysexPayload.size == WAVEFORM_LENGTH) {
      // EMU_printf("SYSEX payload looks correct, updatingwaveform...\n");
      storeWav(dataSet[WAV_Shape], &sysexPayload.data);
      loadWav(wavDataOffset);
    } else {
      //  EMU_printf("SYSEX payload was not correctly formed\n");
    }
  }
}
...
void loadWav(uint8_t offset) {
  ...
  memcpy(&_AUD3WAVERAM, &wavData[offset], WAVEFORM_LENGTH);
